# Maintainers: Chris McGimpsey-Jones <chrisjones.unixmen@gmail.com>, Jan Alexander Steffens (heftig) <heftig@archlinux.org>
...
pkgname=(
  ...
  gst-plugin-msdk
  gst-plugin-onnx
#  gst-plugin-opencv <<< not needed as we are not including opencv
  gst-plugin-qml6
  gst-plugin-qmlgl
  gst-plugin-qsv
  gst-plugin-va
...
pkgver=2.00.1
pkgrel=1
pkgdesc="Patched, Wayland-free, GStreamer2 core framework."
...
#  0002-opencv-5.patch <<< try disabling this to get a good build. then investigate
)
b2sums=('SKIP'
        'SKIP'
        'SKIP'
...
build() {
  local meson_options=(
    ...
    -D libnice=disabled
    -D orc-source=system
    -D package-name="GNU Operating System / H-Linux GStreamer ${epoch:+$epoch:}$pkgver-$pkgrel"
    -D package-origin="https://www.freedompublishersunion.net/h-linux.html"
    # Intentional culls
    -D wayland=disabled
    # Intentional culls, issues raised
...
# Nuke the tests, they are not needed for this build and waste time
#check() (
...
#    wayland
  ...
  cd root; local files=(
    ...
    usr/include/gstreamer-1.0/gst/{analytics,basecamerabinsrc,codecparsers,cuda,insertbin,interfaces,isoff,mpegts,mse,play,player,sctp,transcoder,uridownloader,va,vulkan,webrtc}
    ...
    usr/lib/libgst{photography,play,player,sctp,transcoder,uridownloader,va,vulkan,webrtc,webrtcnice}-1.0.so*
    usr/lib/pkgconfig/gstreamer-{analytics,bad-audio,codecparsers,cuda,insertbin,mpegts,mse,photography,play,player,sctp,transcoder,vulkan-xcb,va,webrtc{,-nice}}-1.0.pc
    usr/lib/girepository-1.0/{CudaGst,Gst{Analytics,BadAudio,CodecParsers,Codecs,Cuda,Dxva,InsertBin,Mpegts,Mse,Play,Player,Transcoder,Va,Vulkan{,XCB},WebRTC}}-1.0.typelib
    usr/share/gir-1.0/{CudaGst,Gst{Analytics,BadAudio,CodecParsers,Codecs,Cuda,Dxva,InsertBin,Mpegts,Mse,Play,Player,Transcoder,Va,Vulkan{,XCB},WebRTC}}-1.0.gir
...
#    wayland
  ...
  cd root; local files=(
    ...
    usr/lib/pkgconfig/gstreamer-{allocators,app,audio,fft,gl{,-egl,-prototypes,-x11},pbutils,riff,rtp,rtsp,sdp,tag,video}-1.0.pc
    usr/lib/girepository-1.0/Gst{Allocators,App,Audio,GL{,EGL,X11},Pbutils,Rtp,Rtsp,Sdp,Tag,Video}-1.0.typelib
    usr/share/gir-1.0/Gst{Allocators,App,Audio,GL{,EGL,X11},Pbutils,Rtp,Rtsp,Sdp,Tag,Video}-1.0.gir
...
package_gstreamer-docs() {
  ...
  # make sure there are no files left to install
    # no, this is nazi and stupid. skipping
